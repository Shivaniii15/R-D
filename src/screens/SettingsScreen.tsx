...
export default function SettingsScreen(): React.JSX.Element {
    ...
    useCallback(() => {
      ...
      async function loadSettings(): Promise<void> {
        ...
        try {
          ...
          setLocalSleepRemindersEnabled(sleepEnabled);
          setSelectedSleepTime(sleepTime);
        } catch (error) {
  ...
  async function handleReminderToggle(enabled: boolean): Promise<void> {
    ...
    try {
      ...
      Alert.alert('Unable to update reminders', 'The self-care reminder setting could not be changed.');
    } finally {
  ...
  async function handleSleepReminderToggle(enabled: boolean): Promise<void> {
    ...
    try {
      ...
      Alert.alert('Unable to update sleep reminders', 'The sleep reminder setting could not be changed.');
    } finally {
  ...
  async function handleSaveTime(): Promise<void> {
    ...
    try {
      ...
      Alert.alert('Unable to save reminder time', 'The selected reminder time could not be saved.');
    } finally {
      setIsUpdatingMood(false);
    }
  }
  ...
  async function handleSaveSleepTime(): Promise<void> {
    ...
    try {
      ...
      Alert.alert('Unable to save bedtime', 'The selected bedtime could not be saved.');
    } finally {
      setIsUpdatingSleep(false);
    }
  }

  function getPickerDate(time: ReminderTime | SleepReminderTime): Date {
    const pickerDate = new Date();
    pickerDate.setHours(time.hour, time.minute, 0, 0);
    return pickerDate;
  }
      ...
      <ScrollView
        ...
        <View style={styles.settingCard}>
          ...
          <Switch
            ...
            thumbColor={remindersEnabled ? '#111111' : '#f4f4f4'}
          />
        </View>
        ...
        <View style={styles.timeCard}>
          ...
          {showTimePicker && (
            <DateTimePicker
              value={getPickerDate(selectedTime)}
              mode="time"
              is24Hour={false}
        ...
        <View style={styles.settingCard}>
          ...
          <Switch
            ...
            trackColor={{ false: '#d5d5d5', true: '#777777' }}
            thumbColor={sleepRemindersEnabled ? '#111111' : '#f4f4f4'}
          />
        </View>
        ...
        <View style={styles.timeCard}>
          ...
          {showSleepTimePicker && (
            <DateTimePicker
              value={getPickerDate(selectedSleepTime)}
              mode="time"
              is24Hour={false}
              display={Platform.OS === 'ios' ? 'spinner' : 'default'}
...
function formatTime(time: ReminderTime | SleepReminderTime): string {
  const date = new Date();
